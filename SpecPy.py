...
class CapturedFrame:
    def __init__(self, frame, objectName, identifier, center, interval, path):
        ...
        if center == None:
            self.center = int(self.height/2)
        if interval == None:
            self.interval = int(self.height/2)
    ...
    def saveFrame(self):
        temp = self.path +"/"+ self.objectName + " %02d"%int(self.identifier) + ".jpg"
...
#                        row[-1] = str(row[-1]) + '\n'
                ...
                for (line, value) in zip(data, inten):
                    if i == 0:
                        line.append(self.objectName + ' ' + self.identifier)
                    else:
...
class GuiWindow:
    ...
    def captureData(self, temp_frame, fig_num):
        ...
        temp = CapturedFrame(temp_frame, self.objectName, str(fig_num), self.pixelLine, self.range, self.path)
        try:        
